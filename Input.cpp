...
{	
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		...
		{	
			...
			case ITM_EXIT: return EXIT;
				...
				/// Add cases for the other items of Design Mode(DONE)
	...
	else	
	{
		...
		case ITM_EXIT_GAME_MODE:return EXIT;
...
CellPosition Input::GetCellClicked() const
{
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		if ( y >= UI.ToolBarHeight && y <= (UI.height - UI.StatusBarHeight))
		{
			/// SetHCell and SetVCell of the object cellPost appropriately (DONE) but related to Class CellPosition
		...
		else { cellPos.SetHCell(-1); cellPos.SetVCell(-1); }
	}
